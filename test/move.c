...
SINGLE_BATTLE_TEST("Accuracy controls the proportion of misses")
{
    ...
    PASSES_RANDOMLY(gBattleMoves[move].accuracy, 100, WRNG_ACCURACY);
    GIVEN {
...
SINGLE_BATTLE_TEST("Secondary Effect Chance controls the proportion of secondary effects")
{
    ...
    PASSES_RANDOMLY(gBattleMoves[move].secondaryEffectChance, 100, WRNG_SECONDARY_EFFECT);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Critical hits occur at a 1/24 rate")
{
    ...
    PASSES_RANDOMLY(1, 24, WRNG_CRITICAL_HIT);
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
