...
void Task_LinkContest_CommunicateRngRS(u8 taskId)
{
    ...
    case 0:
        if (GetMultiplayerId() == 0)
        {
            if (IsLinkTaskFinished() && LinkContest_SendBlock(&gRngState, sizeof(gRngState)) == TRUE)
                gTasks[taskId].tState++;
        }
        else
        {
    ...
    case 1:
        ...
        {
            memcpy(&gRngState, gBlockRecvBuffer[0], sizeof(gRngState));
            memcpy(&gContestRngState, gBlockRecvBuffer[0], sizeof(gContestRngState));
            gTasks[taskId].tState++;
        }
        break;
    default:
